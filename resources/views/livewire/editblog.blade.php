<div class="container mx-auto p-6">
    ...
    @endif
<form wire:submit.prevent="updateblog" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-md">
    @csrf
    <div class="">
    ...
    <div class="mt-4">
        ...
        @if ($thumbnail)
            <div class="mt-2">
                <img src="{{asset('storage/' . $thumbnail)}}" alt=" Thumbnail" class="w-32 h-32 object-cover">
            </div>
        @endif
        <input type="file" wire:model="thumbnail" id="thumbnail"
               class="mt-2 shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
               placeholder="Upload a new thumbnail">
        @error('thumbnail')
            <span class="error text-danger">{{ $message }}</span>
        @enderror
    </div>
